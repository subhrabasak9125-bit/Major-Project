...
class RealTimeStatistics:
    ...
    def get_last_7_days_attendance(self):
        ...
        result = []
        for i in range(7):
            day = datetime.now() - timedelta(days=6-i)  # Start from 6 days ago
    ...
    def get_department_distribution(self):
        ...
        return dept_count if dept_count else {"DCSE": 0}

    def get_weekly_performance(self):
        """Returns present and absent count per week for last 4 weeks"""
        attendance_dir = os.path.join(self.base_dir, "attendance")
        total_students = self.get_total_students()

        weekly_present = [0, 0, 0, 0]  # Week1, Week2, Week3, Week4
        weekly_total_days = [0, 0, 0, 0]  # Count days with records
                ...
                try:
                    ...
                    if 0 <= week_index < 4:
                        with open(filename, "r") as f:
                            ...
                            for row in reader:
                                if row.get("Status") == "Present":
                                    unique_students.add(row.get("StudentID", ""))
                            weekly_present[3 - week_index] += len(unique_students)
                            weekly_total_days[3 - week_index] += 1
                except:
                    pass
        except:
            pass

        # Calculate absent (total students * days - present)
        weekly_absent = []
        for i in range(4):
            if weekly_total_days[i] > 0 and total_students > 0:
                expected_attendance = total_students * weekly_total_days[i]
                absent = max(0, expected_attendance - weekly_present[i])
                weekly_absent.append(absent)
            else:
                weekly_absent.append(0)

        return weekly_present, weekly_absent
    ...
    def get_hourly_attendance(self):
        """Get attendance count by hour for today"""
        today = datetime.now().strftime("%Y-%m-%d")
        file = os.path.join(self.base_dir, "attendance", f"attendance_{today}.csv")
        
        hourly_counts = {f"{h}:00": 0 for h in range(8, 18)}  # 8 AM to 5 PM
